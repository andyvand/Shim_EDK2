...
{
    EFI_STATUS  Status;
    EFI_GUID UNICODE_COLLATION_GUID = gEfiUnicodeCollationProtocolGuid; // { 0xa4c751fc, 0x23ae, 0x4c3e, {0x92, 0xe9, 0x49, 0x64, 0xcf, 0x63, 0xf3, 0x49} };
...
static EFI_STATUS get_sha1sum (void *Data, int DataSize, UINT8 *hash)
{
	...
	ctxsize = (unsigned int)Sha1GetContextSize();
...
static UINT32 count_keys(void *Data, UINTN DataSize)
{
	...
	EFI_GUID CertType = EFI_CERT_X509_GUID;
...
static CHAR16* get_x509_name (X509_NAME *X509Name)
{
	...
	for (i = 0; nidname[i].name != NULL; i++) {
		...
		if (first) {
			UnicodeSPrint(part, NAME_LINE_MAX * sizeof(CHAR16), L"%s=%a",
			       nidname[i].name, str);
		} else {
			UnicodeSPrint(part, NAME_LINE_MAX * sizeof(CHAR16), L", %s=%a",
			       nidname[i].name, str);
...
static void show_mok_info (void *Mok, UINTN MokSize)
{
	...
	if (MokSize != SHA256_DIGEST_SIZE) {
		efi_status = get_sha1sum(Mok, (int)MokSize, hash);
...
static EFI_STATUS list_keys (void *KeyList, UINTN KeyListSize, CHAR16 *title)
{
	...
	while (key_num < (INTN)MokNum) {
		CHAR16 *selection[2];
		selection[0] = title;
		selection[1] = NULL;
		key_num = console_select(selection, menu_strings, 0);

		if (key_num < 0)
			break;
		else if (key_num < (INTN)MokNum)
				   ...
				   UINT32 pw_length, UINT8 *hash)
{
	...
	ctxsize = (unsigned int)Sha256GetContextSize();
	ctx = AllocatePool(ctxsize);
...
static void console_save_and_set_mode (EFI_SIMPLE_TEXT_OUTPUT_MODE *SavedMode)
{
	...
	CopyMem(SavedMode, gST->ConOut->Mode, sizeof(EFI_SIMPLE_TEXT_OUTPUT_MODE));
...
static UINT32 get_password (CHAR16 *prompt, CHAR16 *password, UINT32 max)
{
	...
	console_print_box_at(message, (unsigned int)-1, (unsigned int)(-(int)length-4), (unsigned int)-5, (unsigned int)(length+4), 3, 0, 1);
...
static EFI_STATUS store_keys (void *MokNew, UINTN MokNewSize, int authenticate)
{
	...
	if (authenticate) {
		efi_status = gRT->GetVariable(L"MokAuth",
					       &shim_lock_guid,
	...
	if (!MokNewSize) {
		/* Delete MOK */
		efi_status = gRT->SetVariable(L"MokList",
					       &shim_lock_guid,
...
static UINTN mok_enrollment_prompt (void *MokNew, UINTN MokNewSize, int auth) {
	...
	if (auth) {
		gRT->SetVariable(L"MokNew", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
		gRT->SetVariable(L"MokAuth", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
...
static EFI_STATUS write_back_mok_list (MokListNode *list, INTN key_num)
{
	...
	for (i = 0; i < key_num; i++) {
		...
		ptr = (UINT8 *)ptr + sizeof(EFI_SIGNATURE_LIST) +
...
static EFI_STATUS delete_keys (void *MokDel, UINTN MokDelSize)
{
	...
	efi_status = gRT->GetVariable(L"MokDelAuth",
				       &shim_lock_guid,
				       &attributes, &auth_size, auth);
	...
	if (attributes & EFI_VARIABLE_RUNTIME_ACCESS) {
		console_alertbox((CHAR16 *[]){L"MokList is compromised!",
					...
					NULL});
		if (gRT->SetVariable(L"MokList", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
	...
	mok = build_mok_list((UINT32)mok_num, MokListData, (UINT32)MokListDataSize);
	del_num = count_keys(MokDel, MokDelSize);
	del_key = build_mok_list((UINT32)del_num, MokDel, (UINT32)MokDelSize);
...
static INTN mok_deletion_prompt (void *MokDel, UINTN MokDelSize)
{
	...
	gRT->SetVariable(L"MokDel", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
	gRT->SetVariable(L"MokDelAuth", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
...
static INTN mok_sb_prompt (void *MokSB, UINTN MokSBSize) {
	...
	if (ret == 0) {
		gRT->SetVariable(L"MokSB", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
		return -1;
	}

	if (var->MokSBState == 0) {
		efi_status = gRT->SetVariable(L"MokSBState",
					       &shim_lock_guid,
					       EFI_VARIABLE_NON_VOLATILE |
					       EFI_VARIABLE_BOOTSERVICE_ACCESS,
					       1, &sbval);
		...
		gRT->SetVariable(L"MokSBState", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
	}
...
static INTN mok_db_prompt (void *MokDB, UINTN MokDBSize) {
	...
	if (ret == 0) {
		gRT->SetVariable(L"MokDB", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
		return -1;
	}

	if (var->MokDBState == 0) {
		efi_status = gRT->SetVariable(L"MokDBState",
					       &shim_lock_guid,
					       EFI_VARIABLE_NON_VOLATILE |
					       EFI_VARIABLE_BOOTSERVICE_ACCESS,
					       1, &dbval);
		...
		gRT->SetVariable(L"MokDBState", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
	}
...
static INTN mok_pw_prompt (void *MokPW, UINTN MokPWSize) {
	...
	if (clear) {
		...
		gRT->SetVariable(L"MokPWStore", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
		gRT->SetVariable(L"MokPW", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL);
		console_notify(L"The system must now be rebooted");
		gRT->ResetSystem(EfiResetWarm, EFI_SUCCESS, 0,
				  NULL);
		console_notify(L"Failed to reboot");
		return -1;
	}
	...
	efi_status = gRT->SetVariable(L"MokPWStore",
				       &shim_lock_guid,
				       EFI_VARIABLE_NON_VOLATILE |
				       EFI_VARIABLE_BOOTSERVICE_ACCESS,
				       MokPWSize, MokPW);
...
static EFI_STATUS enroll_file (void *data, UINTN datasize, BOOLEAN hash)
{
	...
	UINTN mokbuffersize;
	void *mokbuffer = NULL;
	char *mokbuffercast = NULL;
	EFI_GUID SHAGUID = EFI_CERT_SHA256_GUID;
	EFI_GUID X509GUID = EFI_CERT_X509_GUID;
	...
	if (hash) {
		...
		status = gBS->LocateProtocol(&shim_guid, NULL, (VOID **)&shim_lock);
		...
		status = shim_lock->Context(data, (unsigned int)datasize, &context);

		if (status != EFI_SUCCESS)
			goto out;
	
		status = shim_lock->Hash(data, (int)datasize, &context, sha256,
					 sha1);
		...
		CertList = mokbuffer;
		CertList->SignatureType = SHAGUID;
		...
		CertList = mokbuffer;
		CertList->SignatureType = X509GUID;
		CertList->SignatureSize = (UINT32)(16 + datasize);

		mokbuffercast = mokbuffer;
		memcpy(mokbuffercast + sizeof(EFI_SIGNATURE_LIST) + 16, data,
		       datasize);
...
static BOOLEAN check_der_suffix (CHAR16 *file_name)
{
    ...
    InitializeUnicodeCollationProtocol();
	mUnicodeCollation->StrLwr (mUnicodeCollation, suffix);
...
static BOOLEAN verify_pw(BOOLEAN *protected)
{
	...
	*protected = FALSE;
	...
	efi_status = gRT->GetVariable(L"MokPWStore",
...
static int draw_countdown()
{
	...
	gST->ConOut->QueryMode(gST->ConOut,
			  gST->ConOut->Mode->Mode, &cols, &rows);
	...
	while (1) {
		...
		status = WaitForSingleEvent(gST->ConIn->WaitForKey, wait);
...
static EFI_STATUS check_mok_request(EFI_HANDLE image_handle)
{
	...
	if (status == EFI_SUCCESS) {
		if (gRT->SetVariable(L"MokNew", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
	...
	if (status == EFI_SUCCESS) {
		if (gRT->SetVariable(L"MokDel", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
	...
	if (status == EFI_SUCCESS) {
		if (gRT->SetVariable(L"MokSB", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
	...
	if (status == EFI_SUCCESS) {
		if (gRT->SetVariable(L"MokPW", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
	...
	if (status == EFI_SUCCESS) {
		if (gRT->SetVariable(L"MokDB", &shim_lock_guid, EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_NON_VOLATILE, 0, NULL) != EFI_SUCCESS) {
          ...
          )
{
	...
	efi_status = check_mok_request(ImageHandle);
