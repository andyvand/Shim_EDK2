...
console_print_box_at(CHAR16 *str_arr[], int highlight, int start_col, int start_row, int size_cols, int size_rows, int offset, int lines)
{
	...
	if (size_rows < 0)
		size_rows = (int)(rows + size_rows + 1);
	if (size_cols < 0)
		size_cols = (int)(cols + size_cols + 1);

	if (start_col < 0)
		start_col = (int)((cols + start_col + 2)/2);
	if (start_row < 0)
		start_row = (int)((rows + start_row + 2)/2);
	if (start_col < 0)
		start_col = 0;
	if (start_row < 0)
		start_row = 0;
	...
	if (size_cols + start_col > (int)cols)
		size_cols = (int)(cols - start_col);
	if (size_rows + start_row > (int)rows)
		size_rows = (int)(rows - start_row);
...
console_select(CHAR16 *title[], CHAR16* selectors[], int start)
{
	...
	for (i = 0; i < selector_lines; i++) {
		int len = (int)StrLen(selectors[i]);
	...
	if (selector_lines > (int)(rows - 10)) {
		int title_lines = count_lines(title);
		offs_row =  title_lines + 1;
		size_rows = (int)(rows - 3 - title_lines);
		lines = size_rows - 2;
	} else {
...
console_error(CHAR16 *err, EFI_STATUS status)
{
	...
	UnicodeSPrint(str, sizeof(str), L"%s: (%d) %s", err, status, err_string(status));
...
VOID setup_console (int text)
{
	...
	status = gBS->LocateProtocol(&console_control_guid, NULL, (VOID **)&concon);
	...
	if (text) {
		...
		status = concon->GetMode(concon, &mode, 0, 0);
