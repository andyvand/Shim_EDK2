...
simple_file_open_by_handle(EFI_HANDLE device, CHAR16 *name, EFI_FILE **file, UINT64 mode)
{
	...
	efi_status = root->Open(root, file, name,
				       mode, 0);
...
simple_file_open(EFI_HANDLE image, CHAR16 *name, EFI_FILE **file, UINT64 mode)
{
	...
	efi_status = gBS->HandleProtocol(image,
		    ...
		    int *count)
{
	...
	status = file->GetInfo(file, &FILE_INFO,
				   &size, fi);
...
simple_file_read_all(EFI_FILE *file, UINTN *size, void **buffer)
{
	...
	*size = sizeof(buf);
	...
	efi_status = file->GetInfo(file, &FILE_INFO,
				       size, fi);
	if (efi_status != EFI_SUCCESS) {
		Print(L"Failed to get file info\n");
		return efi_status;
...
simple_volume_selector(CHAR16 **title, CHAR16 **selected, EFI_HANDLE *h)
{
	...
	for (i = 0; i < count; i++) {
		...
		status = gBS->HandleProtocol(vol_handles[i],
					   &SIMPLE_FS_PROTOCOL,
					   (void **)&drive);
		...
		status = root->GetInfo(root, &FS_INFO,
					   &size, fi);
		if (status != EFI_SUCCESS)
			continue;
...
{
	EFI_STATUS status;
	int tot, offs = (int)StrLen(filter), i, c, filtercount = 1;
	EFI_FILE_INFO *next;
	void *ptr;
	char *ptrcast;
	...
	*count = 0;
	...
	for (i = 0; i < tot; i++) {
		int len = (int)StrLen(next->FileName);

		for (c = 0; c < filtercount; c++) {
			offs = (int)StrLen(filterarr[c]);
		...
		ptrcast = ptr;
		ptrcast += OFFSET_OF(EFI_FILE_INFO, FileName) + (len + 1)*sizeof(CHAR16);
		ptr = ptrcast;
		next = ptr;
	}
	if (*count)
